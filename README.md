...
- **`/admin`** — Excel-подобный редактор любой таблицы БД: правка прямо в ячейках,
  навигация стрелками/Tab/Enter, копирование/вставка диапазонов (в т.ч. из Excel, `Ctrl+C/Ctrl+V`),
  сортировка по заголовку, быстрый фильтр, FK-поля выпадающим списком. Правки копятся и сохраняются
  **пакетом** (кнопка «Сохранить») — в журнале один пакет, откатывается целиком.

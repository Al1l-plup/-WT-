...
**Развёртывание на сервере** (Windows Server + RDP, доступ по локальной сети цеха) —
пошаговая инструкция: [docs/DEPLOY_WINDOWS.md](docs/DEPLOY_WINDOWS.md).
...
- **Тот же редактор встроен во вкладки ТО и Дефекты** («Редактор записей ТО» / «Редактор
  дефектов») — записи фактов правятся прямо в таблице (правка ячеек, копипаст, добавление/
  удаление строк) через общий движок (`app/static/editor-embed.js` + документы `maintenance`/
  `defects`). Все правки — в `change_log` с автором и датой, откатываются.

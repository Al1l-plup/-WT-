...
DOCUMENTS = {
    ...
    'weld_balance': {
        ...
        # Показываем ВСЕ колонки Excel: реальные поля weld_point — редактируемые;
        # «прочие» колонки Excel лежат в raw_extra (JSON) — показываем их read-only (json_extract),
        # чтобы не терять ни одной колонки. (Служебные формулы Excel mark1/2/3 — тоже видны.)
        'columns': [
            ...
            # связи (для справки, read-only) и файл-источник
            ...
            # «прочие» колонки Excel из raw_extra (read-only, ничего не теряем)
